...
DISCORD_GUILD_ID=<任意。guild command登録先のDiscord server ID>
DISCORD_DELETE_COMMAND_ALLOWED_USER_IDS=<実行を許可するDiscord user IDをカンマ区切り>
DISCORD_CLEANUP_FORWARD_SECRET=<openssl rand -hex 32 の出力>
...
```text
INTERNAL_TASK_SECRET: openssl rand -hex 32 の出力
DISCORD_CLEANUP_FORWARD_SECRET: openssl rand -hex 32 の出力。staging / production で同じ値
CORS_ALLOW_ORIGINS:
