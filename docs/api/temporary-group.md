...
`location_id` が指定されている場合は、backendが地点マスタから緯度経度を解決し、
作成時にHot Pepper APIで店舗候補を半径検索して `temporary_groups.restaurant` に保存する。
...
`location` だけが指定され、`location_id` がない場合は400を返す。
