...
ことが難しかった。現在の投票機能も、この保存済み候補をsource of truthとして使う。
...
投票は実装済みで、`temporary_group_votes.restaurant_id` は
`restaurant.restaurants[].id` を参照する。店舗候補は独立テーブルではないため、
DBの外部キーではなくservice層で候補IDを検証する。
